...
const TABS = [
  ...
  },
  {
    ...
    schema: [
      ...
      { key: 'cta.agentSignup', label: 'Button opens agent registration (the agent then lists from their panel)', type: 'toggle' },
      { key: 'cta.link', label: 'Button link (used when the toggle above is off, or the visitor is a client)', placeholder: '/contact?intent=sell' },
    ],
  },
  {
    id: 'about',
  ...
  },
  {
    ...
    schema: [
      ...
      { key: 'contact.waGeneral', label: 'WhatsApp message — floating button & mobile menu', type: 'textarea', rows: 2, hint: 'The text WhatsApp opens with. Tokens: {brand} (your company name).' },
      { key: 'contact.waEnquiry', label: 'WhatsApp message — Contact page', type: 'textarea', rows: 2 },
      { key: 'contact.waThankYou', label: 'WhatsApp message — thank-you page', type: 'textarea', rows: 2 },
      { key: 'contact.waProperty', label: 'WhatsApp message — to the agent on a property page', type: 'textarea', rows: 2, hint: 'Tokens: {property} (the listing title), {brand}.' },
  ...
  legalTab('disclaimer', 'Disclaimer'),
  {
    ...
    schema: [
      ...
      { key: 'agentProgram.listIntro', label: 'Line shown when someone starts from “List My Property”', type: 'textarea', rows: 2 },
  ...
  {
    id: 'welcome',
    label: 'Welcome & exit card',
    target: 'content',
    intro: 'The card a visitor sees when they arrive (invites a login or sign-up with their mobile number) and the “before you go” card shown when they are about to leave the site. Both are dismissible and appear once per visit — never on the admin, agent or dashboard panels. Tokens: {brand}.',
    schema: [
      { key: 'welcome.enabled', label: 'Show the welcome card on arrival', type: 'toggle' },
      { key: 'welcome.delaySeconds', label: 'Wait this many seconds first', type: 'number', step: 1, min: 0 },
      { key: 'welcome.eyebrow', label: 'Small line above the title' },
      { key: 'welcome.title', label: 'Title', half: false },
      { key: 'welcome.text', label: 'Text', type: 'textarea', rows: 3 },
      { key: 'welcome.loginLabel', label: 'Login button' },
      { key: 'welcome.signupLabel', label: 'Sign-up button' },
      { key: 'welcome.closeLabel', label: 'Small “close” link' },
      { key: 'welcome.onClose', label: 'When the visitor closes the welcome card', type: 'select', options: [{ value: 'signup', label: 'Open the sign-up form' }, { value: 'nothing', label: 'Just close it' }], half: false },
      { key: 'welcome.exitEnabled', label: 'Show the “before you go” card when a visitor is about to leave', type: 'toggle' },
      { key: 'welcome.exitEyebrow', label: 'Exit card — small line' },
      { key: 'welcome.exitTitle', label: 'Exit card — title', half: false },
      { key: 'welcome.exitText', label: 'Exit card — text', type: 'textarea', rows: 3 },
    ],
  },
  {
    id: 'options',
