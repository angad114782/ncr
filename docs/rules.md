...
11. **An optimistic API write's success message must wait for the real result, not the optimistic local update.** `makeApiCrud`'s `upsert`/`patch`/etc. update local state immediately and roll it back via `reload()` if the request fails — right for a single edit, where a failure toast plus the row reverting is enough feedback. **A bulk import is different**: it's a big enough action (dozens/hundreds of rows) that the person needs to *know* whether it actually landed before they navigate away, and one bad row 400s the *whole* batch (no partial import — see `propertyImport`/`agentPropertyImport` in `backend/src/lib/schemas.js`, a plain `z.array(...)`). `upsertMany` exposes this as `ready` (the real request promise — `Promise.resolve()` in local/non-API mode, so callers don't need to branch on `USE_API`); `CsvToolbar.jsx` awaits it before showing "Imported" and shows the server's real error (which names the row/field) on failure. Found 2026-09-22 from a live report: 102 CSV rows looked imported (the table updated, "Imported 102" showed) because the local optimistic update always ran and the notice never checked whether the request behind it had actually succeeded — the batch had in fact 400'd and nothing was saved. Any new bulk action needs the same `ready`-and-await treatment, not just a local list update. **Not yet covered**: the agent panel's own CSV import (`AgentListings.jsx`) doesn't call the batch endpoint at all — it fires one `upsert()` per row, each with the same un-awaited-optimistic shape `upsert` still has everywhere else. Same class of issue, smaller blast radius (agent listings start `pending` regardless, so a partial/failed import is a lot less confusing than 102 silently-vanished admin listings was) — worth the same fix if it comes up again.
...
2a. **A signed-in account's history is also kept server-side, for admin.** The same `track()` calls batch to `POST /api/me/events` (API mode, `requireAuth`), which stamps each event with the caller's IP and a best-effort city/region/pincode (`backend/src/lib/geo.js` — free lookup, cached, never blocks or throws on failure). Admin → Users → a row's click (`UserActivitySheet.jsx`, `GET /api/admin/users/:id/activity`) shows that one account's full history, newest first — every view, search, save and compare, with an explicit "estimate, not exact" note on the location. **This never extends to anonymous visitors** — an anonymous visitor's *browsing behaviour* (point 2, above: views, searches, saves, compares, intent) stays on-device only; that timeline is only ever written for a signed-in, consenting account. Admin → **Search Analytics** (`GET /admin/analytics/searches`, §8e) aggregates the same `search` events across everyone — it inherits the identical limitation (signed-in accounts only) and must keep saying so (`signedInOnly: true`) rather than being shown as total site traffic.
2b. **A separate, minimal anonymous visit counter does reach the server** (`Visit` model, `POST /api/visits/ping`, `GET /api/visits/count`, `GET /api/admin/visitors`) — deliberately outside the point-2a limitation, and deliberately narrower: one row per IP per calendar day (never per page, never linked to an account or session), with the same best-effort city/region/pincode as 2a. It exists only for a site-traffic count (public footer badge, `Footer.jsx`) and an admin traffic log (Admin → **Visitors**) — never for personalisation, and it carries no click-level history. The front end pings once per browser session (`VisitPing.jsx`); the admin's own browsing is excluded server-side (`req.user.role === 'admin'`) so the count stays a real-visitor number. Because this changes what visitors are told in `/privacy` (Admin → Site Content → Privacy Policy → "What we collect"), keep that text in sync with this behaviour.
...
3a. **Verifying the lead form's OTP also signs the visitor in.** `POST /auth/verify-phone` (`name` optional in the body) now logs into the existing account for that number, or — a `name` was given — creates one, in addition to returning the `phoneToken` it always did (`LeadForm.jsx` via `useAuth().verifyPhoneAndLogin`). This relies on the enquiry's own consent, not a separate sign-up checkbox — filling the form and confirming the code already is that consent. Never for an agent or admin number (wrong door for a buyer-facing form; same guard as `AuthSheet`'s own login/signup).
