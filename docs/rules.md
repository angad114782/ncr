...
2a. **A signed-in account's history is also kept server-side, for admin.** The same `track()` calls batch to `POST /api/me/events` (API mode, `requireAuth`), which stamps each event with the caller's IP and a best-effort city/region/pincode (`backend/src/lib/geo.js` — free lookup, cached, never blocks or throws on failure). Admin → Users → **Activity** (`UserActivitySheet.jsx`, `GET /api/admin/users/:id/activity`) shows one account's full history, newest first, with an explicit "estimate, not exact" note on the location. **This never extends to anonymous visitors** — an anonymous visitor's behaviour (point 2, above) stays on-device only; the server-side timeline is only ever written for a signed-in, consenting account.
